...
// The task worked with only integers for a 100/100 solution. Doubles may seem more reasonable for this task but hackkerrank said no -__- . Anyway, this solution works with doubles

void FillVector(std::vector<double>& weights, int students);

void MergeSort(std::vector<double>& arr, int startIndex, int endIndex);
void MergeArrays(std::vector<double>& arr, int startIndex, int midIndex, int endIndex);

bool ValidateLimits(double& lower, double& upper);

void PrintBoxersCount(std::vector<double>& weights, int students, const int tournaments);
...
void MergeArrays(std::vector<double>& arr, int startIndex, int midIndex, int endIndex)
{
	...
	std::vector<double>leftArr;
	std::vector<double>rightArr;
...
void PrintBoxersCount(std::vector<double>& weights, int students, const int tournaments)
{
	...
	for (int currMatch = 0; currMatch < tournaments; currMatch++)
	{
		...
		std::vector<double>::iterator startBound = std::lower_bound(weights.begin(), weights.end(), minKilos);
		int currRoundStartIndex = startBound - weights.begin(); // in order to convert it from iterator to integer we do this arithmetic
		std::vector<double>::iterator endBound = std::upper_bound(weights.begin(), weights.end(), maxKilos);
